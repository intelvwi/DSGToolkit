    ...
    private void GetRecordFieldNames(Records rec, ref List<string> fieldNames)
    {
        ...
        if (syncRec != null)
        {
            ...
            */
            ...
            fldName = fldNameBase + " msgsSentPerSec";
            if (!fieldNames.Contains(fldName))
                fieldNames.Add(fldName);
            rec.outputFields.Add(fldName, syncRec.msgs_sent_per_sec.ToString());

            fldName = fldNameBase + " msgsRcvdPerSec";
            if (!fieldNames.Contains(fldName))
                fieldNames.Add(fldName);
            rec.outputFields.Add(fldName, syncRec.msgs_rcvd_per_sec.ToString());

            fldName = fldNameBase + " BytesSentPerSec";
            if (!fieldNames.Contains(fldName))
                fieldNames.Add(fldName);
            rec.outputFields.Add(fldName, syncRec.bytes_sent_per_sec.ToString());

            fldName = fldNameBase + " BytesRcvdPerSec";
            if (!fieldNames.Contains(fldName))
                fieldNames.Add(fldName);
            rec.outputFields.Add(fldName, syncRec.bytes_rcvd_per_sec.ToString());
